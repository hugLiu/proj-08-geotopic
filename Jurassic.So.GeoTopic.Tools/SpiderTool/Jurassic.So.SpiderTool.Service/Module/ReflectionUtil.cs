...
namespace Jurassic.So.SpiderTool.Service
{
    ...
    class ReflectionUtil
    {
        ...
        public static Type GetImplemnetType(Assembly assembly, string interfaceName)
        {
            // 获取类型
            Type[] types = assembly.GetTypes();
            foreach (Type type in types)
            {
                if (type.GetInterface(interfaceName, true) != null && !type.IsAbstract)
                {
                    return type;
                }
            }
        ...
        public static MethodInfo[] GetMethodInfos(Type type, BindingFlags bindingAttrs)
        {
            try
            {
                return type.GetMethods(bindingAttrs); ;
            }
            catch (Exception ex)
            {
                string errMsg = ex.Message;
        ...
        public static MethodInfo GetMethod(Type type, BindingFlags bindingAttrs, string methodName)
        {
            try
            {
                MethodInfo[] methods = type.GetMethods(bindingAttrs);
        ...
        public static ParameterInfo[] GetMethodParameters(MethodInfo m)
        {
        ...
        #region 动态调用方法
        public static object InvokeMethod(string assemblyFile, string className, string method, object[] args)
        {
            try
            {
                ...
                return t.InvokeMember(method, defaultBinding, null, instance, args);
            }
            catch (Exception ex)
            {
                string errMsg = ex.Message;
