...
function getSubmitButtons() {
    _submitButtons = _submitButtons || $('form').find('.frame-submit');
}
...
$(function () {
    ...
    $('form').each(function () {
        ...
        //将流程信息和按钮本身的提示文本加到表单中一起提交给服务器
        if (form.children("[name='submitValue']").length == 0) {
            form.append("<input type='hidden' name='submitValue' value='' />")
            form.append("<input type='hidden' name='Flow_Status' value='' />")
            form.append("<input type='hidden' name='Flow_Remark' value='' />")
        }
        ...
        _submitButtons.click(function (e) {
            ...
            form.children("[name='submitValue']").val(sv);
            form[sv] = this;
            if (this.id) {
        ...
        form.submit(
            function (e) {
                e.preventDefault();
                var sv = form.children("[name='submitValue']").val();

                //if (!sv) return;
                var miniButton = getUButton(form[sv]);
                //将流程表单中的数据加到现有form中
                form.children("[name='Flow_Status']").val($('#Flow_Status').val());
                form.children("[name='Flow_Remark']").val($('#Flow_Remark').val());
...
function showTips(tips) {
    ...
    //debugger;
    //var tipsDiv = $('#divTips');
    //if (tipsDiv.length == 0) tipsDiv = parent.$('#divTips');
    //tipsDiv.addClass('hidden');
    //var hdnTipsType = tipsDiv.find(":input[type='hidden']");
    //tips = tips || TipsMessage;
    //if (tips) {
    //    tipsDiv.find('strong').html(tips.Title || "");
    //    tipsDiv.find('span').html(tips.Message || "");
    //    hdnTipsType.val(tips.Type || "");
    //}

    //var tipsType = hdnTipsType.val();
    //if (!tipsType) return;
    //tipsDiv.removeClass("alert-danger alert-warning alert-success alert-info");
    //switch (tipsType) {
    //    case "error": tipsDiv.addClass('alert-danger');
    //        if (tips.Details) {
    //            tipsDiv.find('span').append("&nbsp;<a href='#'>详情...</a>");
    //            tipsDiv.find('span a').click(showDetails);
    //        }
    //        break;
    ...
    //        _time_out = setTimeout(function () { tipsDiv.addClass('hidden'); hdnTipsType.val(''); }, 3000);
    //        break;
    //}
    //tipsDiv.removeClass('hidden');
    //原方法注释掉，尝试改写信息提示方式
    if (!tips || (!tips.Title && !tips.Message)) return;
    switch (tips.Type) {
        case "error":
